...
---
접근법
    타입을 4개로 나눴는데, ㅇ는 마실 잔이고 ㅁ는 마시지 않는 잔이라면,
    T1 : ㅁㅁ
    T2 : ㅇㅁ
    T3 : ㅁㅇ
    T4 : ㅇㅇ
    이렇게 4개로 나누고, 각자의 경우 그 앞에 올 수 있는 타입들을 정의해서 결론은 아래와 같은 점화식을 만듦

    p : 타입 (1~4)
    i : 포도주 순서
    w[i] : i번째 포도주의 양
    dp[p][i] =
        max(dp[3][i-2], dp[4][i-2]) if p = 1
        w[i-1] + max(dp[1][i-2], dp[2][i-2], dp[3][i-2]) if p = 2
        w[i] + max(dp[2][i-2], dp[3][i-2], dp[4][i-2]) if p = 3
        w[i] + w[i-1] + max(dp[1][i-2], dp[2][i-2]) if p = 4
"""
import sys
